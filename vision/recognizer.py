...
def getDots (source, windowname, low, high, treshold, max_treshold):

	source = cv2.cvtColor(source, cv2.COLOR_BGR2HSV)
	
	# source is BGR file, we convert here to HSV
...
def simpleArray (array, width, maxRowOffset):
	...
	# first parameter is array coordinates from getDots function
	# second we can get with image.shape[1]
	# third is constant, try 400
